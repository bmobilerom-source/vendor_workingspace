...
import com.bmobile.workingspace.utils.isServiceRunning
...
class SessionService : Hilt_SessionService() {
    ...
    private fun startGameSession(packageName: String) {
        ...
        sidebar.onSessionStart(packageName)
    ...
    companion object {
        ...
        fun start(context: Context, app: String) {
            // Always deliver the intent, even if the service is already running from a
            // previous session: startGameSession() below already de-dupes on
            // currentPackage == packageName, and switches sessions when a *different*
            // registered app is foregrounded. Gating this on isServiceRunning() used to
            // silently drop every switch-to-a-different-app event once the service had
            // been started once, so neither the game bar nor the per-app game mode was
            // ever re-applied after the very first session.
