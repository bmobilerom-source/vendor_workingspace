 ...
 */
package com.bmobile.workingspace.ui.viewmodel
...
import com.bmobile.workingspace.data.*
...
class PerAppSettingsViewModel @Inject constructor(
    ...
    fun updatePreferredMode(mode: Int) {
        preferredMode = mode
        if (gameModeUtils.activeGame?.packageName == packageName) {
            // This app currently has a live session (game bar is up) — push the mode to
            // GameManager immediately instead of only persisting it for the next launch,
            // so the change is actually felt right away rather than feeling cosmetic.
            gameModeUtils.setActiveGameMode(systemSettings, mode)
        } else {
